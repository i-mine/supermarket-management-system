...
import models.{Merch, MerchInfoSchema}
...
class MerchDaoImpl @Inject()()(dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfig[JdbcProfile] with MerchInfoSchema with MerchDao {
	...
	override def listAll(args: String*): Future[Seq[Merch]] = {
		if (args.length == 3) {
			...
			val likeArg = "'%"+ arg + "%'"
			println(likeArg)
			if(arg.equals("null")){//初次加载页面或者没有输入搜索参数时，返回所有
	...
	override def update(newMerch: Merch): Future[String] = {
		db.run(merchInfos.filter(_.merchId === newMerch.merchId).update(newMerch)).map(
